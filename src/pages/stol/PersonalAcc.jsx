  ...
  const fetchUserData = useCallback(async (token) => {
    try {
      ...
      const response = await axios.get(
        "http://192.168.1.7:8080/api/orders/sales-report",
        {
  ...
  return (
    <div className="personal-acc-container">
      <header className="personal-acc-header">
        <Link to="/stol" className="personal-acc-btn personal-acc-btn-home">
          ←
        </Link>
        <button
          onClick={logout}
          className="personal-acc-btn personal-acc-btn-logout"
        >
          <span className="personal-acc-btn-text">Выйти</span>
          <img
            src={closeTokenIcon}
            alt="Logout"
            className="personal-acc-btn-icon"
          />
        </button>
      </header>
