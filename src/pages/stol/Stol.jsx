...
const Stol = () => {
  ...
  const fetchTables = async () => {
    try {
      const response = await axios.get(
        "http://192.168.1.7:8080/api/tables/all"
      );
  ...
  useEffect(() => {
    ...
    socket.on("table_status", ({ tableId, isActive, workerId }) => {
      console.log(
        `Получено обновление статуса стола ${tableId}: ${
  ...
  const handleTableClick = async (table) => {
    if (table.isActive && table.workerId !== currentWorkerId) {
      return;
    }
    const token = JSON.parse(localStorage.getItem("access_token"));
    if (!token) {
    ...
    try {
      ...
      if (
        tableData.isActive &&
        tableData.workerId &&
        tableData.workerId !== currentWorkerId
      ) {
          ...
          <div
            ...
            className={`stol__card 
              ${
                !table.isActive
                  ? "free"
                  : table.workerId === currentWorkerId
                  ? "mine"
                  : "occupied"
              } 
              ${
          ...
          >
            ...
            <div className="stol__card-body">
              ...
              {table.isActive &&
                table.workerId &&
                table.workerId !== currentWorkerId && (
                  <span className="stol__occupied-by">
